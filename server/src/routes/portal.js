...
router.post('/generate/:customerId', authenticateToken, async (req, res) => {
  try {
    ...
    await runQuery('DELETE FROM portal_tokens WHERE customer_id = ? AND gym_id = ?', [customerId, gymId]);
...
router.get('/view/:token', async (req, res) => {
  try {
    ...
    const portalToken = await getOne(
      `SELECT * FROM portal_tokens WHERE token = ? AND (expires_at IS NULL OR expires_at > NOW())`,
      [token]
    );
    ...
    const gym = await getOne('SELECT name, logo, color_theme, phone FROM gyms WHERE id = ?', [gymId]);
    ...
    const customer = await getOne(
      `SELECT name, photo, membership_type, membership_start, membership_end, status,
       ...
       FROM customers WHERE id = ? AND gym_id = ?`,
      [customerId, gymId]
    );
    if (!customer) return res.status(404).json({ error: 'Customer not found' });
    ...
    const [recentAttendance, recentPayments, progressPhotos] = await Promise.all([
      getAll(`SELECT check_in, check_out FROM attendance WHERE customer_id = ? ORDER BY check_in DESC LIMIT 10`, [customerId]),
      getAll(`SELECT amount, payment_date, membership_type, payment_method FROM payments WHERE customer_id = ? ORDER BY payment_date DESC LIMIT 5`, [customerId]),
      getAll(`SELECT photo_data, taken_at, angle, notes, weight FROM progress_photos WHERE customer_id = ? ORDER BY taken_at DESC LIMIT 4`, [customerId]),
    ]);

    res.json({ gym, customer, days_until_expiry: daysUntilExpiry, recent_attendance: recentAttendance, recent_payments: recentPayments, progress_photos: progressPhotos });
  } catch (err) {
...
router.get('/my/:customerId', authenticateToken, async (req, res) => {
  try {
    ...
    const portalToken = await getOne(
      `SELECT token FROM portal_tokens WHERE customer_id = ? AND gym_id = ? AND (expires_at IS NULL OR expires_at > NOW())`,
      [customerId, gymId]
    );
